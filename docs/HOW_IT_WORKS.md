...
## Final popcount firmware

`programs/popcount.asm` is the final nontrivial demonstration firmware. It reads one byte from input port `0xEE`, counts the bits whose value is one, and writes that count from `0` through `8` to output port `0xEF`. This operation is commonly called a population count, or popcount.

For example, `0xA5` is `10100101` in binary. Four positions contain a one, so the firmware produces `0x04`:

```text
input  = 0xA5 = 10100101
output = 0x04 = four set bits
```

The complete source combines constants, memory, a loop, a subroutine, explicit stack data, bitwise logic, a shift, and memory-mapped I/O:
...
The program image occupies addresses `0x00` through `0x2B`, for a total of 44 bytes. Its three mutable values are deliberately stored well after the program and before memory-mapped I/O:

| Address | Symbol | Purpose |
| --- | --- | --- |
| `0xD0` | `WORK_VALUE_ADDRESS` | Copy of the input that is shifted right once per iteration. |
| `0xD1` | `BIT_COUNT_ADDRESS` | Number of one bits found so far. |
| `0xD2` | `BITS_REMAINING_ADDRESS` | Loop counter, initialized to eight and decremented to zero. |

Initialization copies the input to `0xD0`, clears the result at `0xD1`, stores eight at `0xD2`, and keeps the constant one in register `B`. The loop does not stop when the work value becomes zero; it always performs exactly eight iterations, so leading zero bits are handled consistently.

Each iteration calls the subroutine at `0x1C`. `CALL` first pushes the return address `0x10` at stack address `0xFF`. The subroutine loads the work value and `PUSH A` stores that byte at `0xFE`, below the return address. `AND A, B` then isolates bit 0 because `B` contains `0x01`:
...
When the result is zero, `JZ` skips the increment. When it is one, the three instructions `LDA`, `ADD`, and `STA` increase the stored bit count. `POP A` restores the unmasked work value, `SHR A` moves its next bit into position 0, and `STA` saves it for the following iteration. Finally, `RET` removes `0x10` from the stack and resumes the caller. The stack therefore returns to empty after every iteration.

After the subroutine, the main loop subtracts one from the remaining-bit counter. `JNZ` returns to `count_loop` while that counter is nonzero. After the eighth iteration, the program loads the completed count, stores it in the output latch, and executes `HALT` at `0x1B`. Fetching that final opcode advances `PC` to `0x1C`, decimal 28.
...
The simplified cycle count can be derived rather than memorized:

- initialization uses 7 instructions;
- each of the 8 iterations uses 13 instructions when its tested bit is zero;
- each set bit adds the 3 counter-increment instructions;
- output and `HALT` use 3 instructions.

Therefore:

```text
cycles = 7 + (8 * 13) + (3 * set-bit count) + 3
cycles = 114 + (3 * set-bit count)
```

The main test vectors are:

| Input | Binary form | Output | Cycles | Final zero flag |
| --- | --- | --- | ---: | --- |
| `0x00` | `00000000` | `0x00` | 114 | set |
| `0xA5` | `10100101` | `0x04` | 126 | clear |
| `0xFF` | `11111111` | `0x08` | 138 | clear |

All three executions finish with `B = 0x01`, `SP = 0x00`, carry clear, `PC = 0x1C`, work value zero, and the remaining-bit counter zero. The dedicated integration test also checks these internal invariants, rather than checking only the visible output.

The same source and generated binary are available through focused Make targets:
...
The monitor provides a practical way to inspect the boundary between the caller and subroutine:
...
At that breakpoint, initialization has completed, `CALL` has run, `SP` is `0xFF`, and memory `0xFF` contains return address `0x10`. Stepping through `PUSH A` later moves `SP` to `0xFE`, illustrating how subroutine return addresses and explicitly pushed data share the same bounded stack.
...
Assembles and runs `programs/popcount.asm` with a selected virtual input. The resulting set-bit count appears in the output port.
...
Assembles and traces the popcount firmware, including its loop, subroutine calls, stack operations, and final output.
...
Assemble the popcount firmware and open it in the monitor:

```bash
make monitor-popcount
```

Assembles the popcount firmware and opens it in the monitor. Set its input with the monitor's `input <byte>` command before running.
...
Assembles both demonstration programs and runs all automated unit, integration, and process-level tests. Dedicated tests verify every current opcode-to-mnemonic mapping, rejection of an unknown opcode, observer delivery, exact trace and CPU-state formatting, memory-map boundaries, port direction, port reset, strict host-byte parsing, CLI selection, and the monitor command loop. Monitor tests cover stepping, bounded running, reset, mapped-memory display without wraparound, safe binary replacement, persistent breakpoints and trace state, invalid arguments, end-of-file, and overlong input. The original assembled-program test verifies `build/demo.bin`. The popcount integration test executes `build/popcount.bin` with inputs `0x00`, `0xA5`, and `0xFF`, then checks visible results and internal invariants. `tests/test_vm8_process.sh` launches the real executable and also checks popcount execution and its final trace entry.
...
Generate and inspect the final popcount firmware instead:
...
`build/popcount.bin` must contain 44 bytes. `make inspect-popcount` applies the same `wc -c` and `od -An -tx1 -v` checks shown above, so it verifies both the size and exact byte encoding without treating the binary as text.
...
- The popcount firmware ties the architecture together: it reads mapped input, loops over eight bits, stores mutable state in memory, calls a stack-using subroutine, and writes mapped output.
- A fixed eight-iteration loop handles every input byte; its simplified execution time is `114 + 3 * popcount(input)` cycles.
