...
## A aplicação gráfica: VM8 Studio

O VM8 Studio acrescenta uma interface desktop em C++17/FLTK ao redor da mesma CPU e do mesmo montador em C17. Ele está disponível como um `.exe` para Windows x64, um AppImage para Linux x86-64 e um instalador `.deb` para Ubuntu/Debian. Os pacotes Linux requerem glibc 2.36+ e um ambiente gráfico X11 ou XWayland; o `.deb` adiciona **VM8 Studio** à busca de aplicativos e não precisa de FUSE. O usuário final não precisa de WSL, compilador nem conexão com a Internet. O desenvolvimento por Vim, Make e pela interface de linha de comando continua disponível de forma independente.

O caminho integrado é: **texto do editor -> montador de duas passagens na memória do hospedeiro -> imagem de bytes -> CPU simulada**. O montador não chama um programa externo nem exige um arquivo intermediário em disco. A linha do código-fonte associada a cada byte é registrada fora da memória simulada, permitindo destacar instruções e definir breakpoints sem ocupar nenhum dos 238 bytes de programa/dados do VM8. Uma montagem malsucedida informa o erro e sua linha, sem executar silenciosamente um bytecode antigo.

Como exemplo prático, abra **Examples > Popcount**, use Input `0xA5` e clique em **Assemble**, depois em **Run**. A imagem montada ocupa 44 bytes, enquanto o laço executado leva 126 ciclos de instrução e escreve `0x04` na porta de saída. A interface gráfica mostra PC como `0x1C`, equivalente ao 28 decimal da CLI. **Step** executa uma instrução; **Pause** interrompe a execução automática; **Reset** recarrega o programa. Cada Run possui um limite de segurança de 10.000 instruções.

O Studio mostra **Virtual time** e **Real elapsed** separadamente. O tempo virtual é `ciclos de instrução / frequência ativa em Hz`. O valor inicial de **Clock (Hz)** é `1000000` (1 MHz), portanto os 126 ciclos do Popcount representam `126 / 1000000 = 0,000126` segundo, exibidos como `126.000 us` (microssegundos). Aplique `1000` Hz na linha do clock para recalcular os mesmos ciclos como `126.000 ms`, ou `2000000` Hz para obter `63.000 us`. Apply aceita inteiros decimais de 1 até 1000000000 Hz; **Active** confirma o valor em uso. Editar o campo sem Apply não altera o clock ativo. Alterá-lo recalcula todos os ciclos acumulados, e não um histórico de mudanças de frequência. A CPU continua contando um ciclo por instrução tentada, incluindo HALT e erros; o tamanho em bytes é independente dessa contagem.

**Real elapsed** usa um cronômetro monotônico do hospedeiro: Run acumula o tempo ativo, incluindo as esperas de Speed e o trabalho da interface/rastreamento realizado durante a execução. Pause, HALT, erros, breakpoints, o limite de segurança e edições do código o interrompem; Run retoma a acumulação. Step soma apenas o intervalo da execução da instrução e da formatação do rastreamento, sem o tempo que você espera entre cliques nem a atualização posterior da tela. Isso não é um benchmark exclusivo da CPU. **Speed** altera o ritmo da visualização, não o clock virtual. Experimente Observe e Fast com a mesma entrada do Popcount, usando Reset entre as execuções: ambos produzem 126 ciclos e 126 us a 1 MHz, enquanto Fast normalmente apresenta um tempo real menor.

Reset, montagem bem-sucedida, New, Open e carregamento de exemplo zeram os dois tempos, mas mantêm o clock ativo nesta sessão. Uma montagem malsucedida preserva o estado pausado da CPU e dos tempos, impedindo a execução de bytecode antigo. Aplicar Input ou limpar o log não zera os tempos. Reiniciar o Studio restaura o padrão de 1 MHz. A exibição usa ns, us, ms ou s; as casas decimais arredondadas não garantem essa precisão de medição. Esse modelo simples de tempo virtual não pretende reproduzir a temporização de hardware físico nem executar em tempo real na frequência selecionada.

A aba **Assembled bytes** mostra os bytes reais e suas linhas de origem, enquanto a mensagem de montagem informa a quantidade de bytes: são as mesmas inspeções básicas de `od` e `wc`, sem abrir um terminal. Os exemplos de shell apresentados adiante continuam válidos ao trabalhar com arquivos `.bin` pela CLI. **File > Export binary** é opcional e produz bytecode VM8, não um executável desktop.

Pressione **F1** para abrir a ajuda offline com busca, incluindo a sintaxe, o opcode, o efeito compacto, a explicação, as flags e os exemplos de cada instrução. A referência é incorporada da ajuda existente da CLI durante a compilação. Consulte [o guia de uso do Studio](STUDIO.md) para operações com arquivos, atalhos, breakpoints, compatibilidade e recompilação.
